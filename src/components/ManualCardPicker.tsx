...
import { getPositionLabel, Locale, UI_TEXT } from "@/lib/i18n";
        ...
        {positions.map((position, index) => {
          ...
          return (
            <div key={position.id} className="flex min-w-0 flex-col items-center gap-2">
              ...
              {drawn ? (
                <TarotCardView {...drawn} compact locale={locale} />
              ) : (
      ...
      {nextPosition && (
        <div className="flex w-full flex-col items-center gap-3">
          <p className="text-center text-sm text-violet-200" aria-live="polite">
            {text.chooseCard(getPositionLabel(nextPosition, locale))}
          </p>
            ...
            {deck.map(({ card }, index) => {
              ...
              return (
                <button
                  ...
                  aria-label={text.cardAria(index + 1)}
          ...
          <p className="text-xs text-violet-400">
            {text.selected(selectedIndexes.length, positions.length)}
          </p>
        </div>
      )}
    </section>
