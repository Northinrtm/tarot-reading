...
Запрещено начинать текст со вступления или мета-комментария о самом раскладе, вопросе или процессе гадания. Категорически нельзя использовать фразы и их аналоги: "давайте разберёмся", "что говорит нам расклад", "начнём с анализа карт", "вы обратились ко мне", "ваш вопрос имеет для меня значение", "итак". Первое же предложение текста должно называть первую карту расклада и её значение — без какого-либо предисловия перед этим.

Для каждой позиции расклада явно назови выпавшую карту и объясни, что она означает в этой позиции (например: "На позиции «${reading.spread.positions[0].label}» выпал(а) [название карты] — это говорит о..."), а затем свяжи это значение с ответом на вопрос. Ни одна карта не должна остаться без объяснения. Напиши единый текст без списков и заголовков, 200-300 слов.`;
}
